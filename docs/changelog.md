...
- 特性：JSON 批量校对结果改为 `文档.proofread.N.json`；新增重叠校对（用上一轮全文作 target 再校一轮）
- 行为：`proofread file` 只做第 1 轮；`overlay proofread file` 做第 2 轮及以后；面板按钮按文件状态启用，续跑时文案为「… - 续跑」
- 行为：同一轮不能有多份结果。已有第 2 轮及以后时 `proofread file` 拒绝。重叠校对升号只要求最高轮完整；续跑未完成的最高轮时需要紧邻上一轮存在且完整
- 说明：比较前后差异为原稿 vs 最新一轮；要从原稿重跑请删除校对结果文件
...
- 优化：proofread selection 记住上次各步骤选择，作为下次默认（工作区内）
...
- 更新文档
- 特性：在按钮提示中显示对应命令
- 优化：`mark titles from table of contents` 匹配时忽略行头任意数量的 `#` 标记，并按目录级别重写（可大于 6 级），便于调整已标记标题
