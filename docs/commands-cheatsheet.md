...
| **切分与合并** | 校对面板第二块 | 选主文件 → 切分 → 合并 JSON；切出 JSON 后可「准备参考资料」「LLM 校对 JSON」「重叠校对 JSON」 |
...
| 5. 批量校对 | **「LLM 校对 JSON」**（第 1 轮）→ 完成后 **「重叠校对 JSON」**（第 2 轮及以后）→ 结果在「校对结果」 |
...
| 批量校对 JSON | 切分与合并区块 **「LLM 校对 JSON」**（第 1 轮）/ **「重叠校对 JSON」**（用上一轮结果再校） |
...
| **proofread.timeout** | 单次 API 请求超时（**秒**），默认 180 |
