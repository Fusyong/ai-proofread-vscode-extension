...
一个半自动的持续工作流。从当前位置开始，自动切分一段并（若存在）带 `.proofread/examples.md` 作为 reference 提交 LLM 校对，**人工复核校对结果**；接受写回后与选段校对**共用** `editorial-memory.md` 更新逻辑（不再弹出样例勾选 Webview，也不再自动写入 `examples.md`）。
...
2. 在第一轮对话中提交`<reference>${reference}</reference>`和`<context>${context}</context>`；若启用编辑记忆，还会附带 `<editorial_memory>`、`<editorial_memory_recent>`、`<editorial_memory_reference>`、`<current_proofread_context>`；
...
1. 持续发现与监督校对工作流改进：选中一段，通过命令开始校对；LLM梳理上一次校对的改动和拒绝改动，整合出新的层级记忆（与文档结构层级一致）；LLM参考层级记忆校对当前选段；用户审核后回填。
...
10. 校对样例文件改为JSON，配编辑、勾选应用界面（webview）
11. 勘误表改为JSON加web viewer
12. 预置更多提示词，包括常用的专项校对
    1. 典型错误举例校对
        1. 标点符号用法错误
        2. 数字用法错误
        3. 年代、时间错误
