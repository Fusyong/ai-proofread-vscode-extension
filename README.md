...
2. **一般的语言文字校对依赖丰富的知识、语料，建议使用大规模、非推理模型。bug: 某些推理模型、混合模型可能因为运行时间过长而导致错误，而服务器端可能已经实际运行、计费。**
3. 长文本建议先切分后校对
...
11. [ ] 生成、提交校对记录
12. [ ] 内部git版本管理
13. [ ] 推理模型无法使用的问题（可能单纯是因为运行时间过长）
14. [ ] 在按长度切分的基础上调用LLM辅助切分（似乎仅仅在没有空行分段文本上有必要）
15. [ ] 支持Copilot（尝试过一次，回文说API还没有开放。还需要研究参考项目。）
