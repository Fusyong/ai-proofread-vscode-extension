...
1. 分词连写检查提示词
2. 更新拼音规范
3. ~~在知识核查中集成（模型）web搜索服务~~ → 已集成可选 **维基百科/Wikidata API** 通道（`wikipedia` 资料来源；通用 web 搜索仍 TODO）
4. 一个memo管理智能体
    1. 三段式：原文；改后；说明
    2. 分类、加标签存储（字词层为穷尽举例式、句段层为举例加格式说明）
    3. 存档、压缩工作流……
5. 本地视觉模型图像校对工作流（专注于图像审查、版式和空间关系）
6. 尝试用思考模型校对
7. 预置更多提示词
...
9.  读秀在线引文核对
10. 人看的相似度使用编辑距离：fastest-levenshtein，在对齐完成后，快速计算两个相似句子的“修改程度”百分比。它是 JS 环境下编辑距离运算的最快实现
