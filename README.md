...
从当前位置自动带样例校对一段，人工复核保存后收集样例，人工确认后继续下一轮，直到用户中断。

1. 打开要校对的文档，将光标置于起始位置
2. 使用命令 `AI Proofreader: continuous proofread` 或快捷键 `Ctrl+Alt+P` 启动
3. 首次使用需选择切分方式（按长度 / 按标题）、上下文构建方式、温度、提示词重复模式；本流程内自动沿用，**终止后配置清除**，下次启动重新询问
4. 每段校对完成后会打开 diff 视图，可编辑右侧结果
5. **Alt+Enter** 或 **Ctrl+Enter** 接受并写回文档，**Ctrl+Alt+S** 跳过，**Ctrl+Alt+Q** 停止；**关闭 diff 窗口时会提示「接受并继续」或「放弃」**；状态栏可点击停止
6. 接受后若有可收集的样例，会弹出 Webview 供勾选保留（每条用 jsdiff 展示变化）
7. 确认后自动进入下一段；直到文档末尾或执行「停止持续校对」时结束

切分方式在首次使用时选择（按长度或按标题），选择后固定；按长度时使用 `defaultSplitLength`，按标题时使用 `defaultTitleLevels`。
...
1. 在线引文核对
    ...
    2. 中华经典古籍库，
    3. 识典故古籍
2. 人看的相似度使用编辑距离：fastest-levenshtein，在对齐完成后，快速计算两个相似句子的“修改程度”百分比。它是 JS 环境下编辑距离运算的最快实现
3. 调研集成opencc-js，用于繁简转换和用词习惯检查
...
- 特性：持续发现与监督校对
  - 从光标位置按长度自动取段，带样例（如有）调用 LLM 校对
  - diff 复核后 Ctrl+Enter 接受写回、Ctrl+Alt+S 跳过
  - 接受后自动收集待选样例，Webview 用 jsdiff 展示每条变化供勾选
  - 首次配置（上下文、温度、重复模式）后固定，后续轮次自动沿用
  - 快捷键 Ctrl+Alt+P 启动；校对面板与右键菜单增加「持续校对」入口
