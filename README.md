...
6. [x] 用webwiew panel的持久面板代替切分和校对结果通知，可重新打开；
    ...
    2. [x] 重用webwiew报错
    3. [x] 呈现切分摘要信息，如段落数、超长段落等
    4. [x] 查看日志文件时自动滚动到底部
7.  [x] JSON批量提交LLM前展示所有参数，请用户确认或取消
8.  [x] 重构校对进程信息面板：增加进度条（用target原始顺序、字符数对应长度、绿色表示完成、黄色表示已经提交等待结果、灰色表示未提交），重构取消机制（停止新的提交、等待已经提交项返回结果）
9.  [x] 调用pdftotext把PDF文件转换为文本
10. [ ] 预置更多提示词，包括常用的专项校对
    1. [ ] PDF/OCR纯文本整理
    2. [ ] 练习题就地回答
    3. [ ] 翻译
    4. [ ] 按小学语文教材标准加拼音
...
14. [ ] 生成、提交校对记录
15. [ ] 内部git版本管理
16. [ ] 推理模型无法使用的问题（可能单纯是因为运行时间过长）
17. [ ] 在按长度切分的基础上调用LLM辅助切分（似乎仅仅在没有空行分段文本上有必要）
18. [ ] 支持Copilot（尝试过一次，回文说API还没有开放。还需要研究参考项目。）
