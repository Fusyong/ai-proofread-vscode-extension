...
3. 同一区块点 **LLM 校对 JSON**，批量校对切分好的片段（结果为 `文档.proofread.1.json`）。第 1 轮完成后可用 **重叠校对 JSON**，用上一轮全文再校一轮。
...
| **核对选中引文** | 对选中文本做与全文相同的相似度核对（去掉行首 `>`）→ 侧栏「引文核查」 |
| **核对全文引文** | 自动抽取引号/`>` 后批量相似度匹配 → 侧栏「引文核查」（须先建立参考资料索引） |
...
2. PDF导出的文本，如果没有使用空行分段，无法切分，可以使用整理段落命令`format paragraphs`中的“段末加空行”选项加以处理。活文字 PDF 经 `pdftotext`（尤其 layout 模式）转换后，页与页之间会插入分页符 `\f`：典型形态是「页脚页码行（空格+数字）→ `\f` → 下页正文」，文末也常残留末页页码加 `\f`。可用命令 `replace form feeds`（校对面板「替换分页符」）换成带页码的 HTML 注释（`<!--  page 1 -->`）或二级标题（`## page 1`，便于按页切分），也可自定义模板（`\p` 表示页码，留空则删除，`\n` 为换行）；默认可一并去掉 `\f` 前的页脚页码行；可指定起始页码（可为负）、从第几个分页符起处理、每隔几个处理一次。
...
校对面板第二块：先 **选择主文件**（或从工作区选择），再 **切分文档**。切出 JSON 后露出配套文件，以及 **合并 JSON**、**准备参考资料**、**LLM 校对 JSON**、**重叠校对 JSON**。

#### 3.2.1. 文档切分

我的经验，在一般语言文字和知识校对场景中，过短切分往往徒增 token 而不明显提升效果；默认切分长度为 **1400** 字符。标题加长度切分时，长度阈值与最小长度分别按切分长度的 **1.5** 倍与 **0.2** 倍推导（默认即 2100 / 280）。一本十来万字的书稿大约切成七十多段，再交给大模型校对。
...
2. **按标题切分** (Split File by Title)，可输入标题级别（如：1,2），适合有标题且题下正文长度合适（建议不要明显超过长度阈值，默认约 2100）的文档。
3. **按标题和长度切分** (Split File by Title and Length)，适合有标题，但题下正文过长的情况。可配置标题级别与切分长度；长度阈值、最小长度默认由切分长度 × 比例（1.5 / 0.2）得出，也可在交互中覆盖。
...
还会生成日志文件（`.log`），记录切分统计信息，并摘要呈现在校对面板中；**如有过长（如超过长度阈值，默认约 2100 字符）片段时，可以手动加空行分段，然后再次切分。**

切分完成后，本区块会列出 JSON / JSON.md / 切分日志，并可 **比较前后差异**（用分割线表示切分位置）、**合并 JSON**、**准备参考资料**（进入资料检索分支）、**LLM 校对 JSON** / **重叠校对 JSON**（结果出现在「校对结果」区块；面板失效后也可打开 JSON 用右键菜单开始校对）。
...
- 核对选中引文：`Verify Selected Citation（核对选中引文）`（选区去掉行首 `>` 后与全文同一套相似度匹配；结果在「引文核查」）
...
- 续跑：资料准备、意图检索、检索面板选段若已有过程文件，可选择继续上次（追加 1 轮）或重新开始；JSON「继续未完成部分」按过程记录跳过已完成条目
...
JSON 批量校对的按钮在 **切分与合并** 区块（「LLM 校对 JSON」「重叠校对 JSON」）；**选段校对**在校对面板底部。进度、未完成提示、比较前后差异、生成差异文件 / 勘误表在 **校对结果** 区块。比较差异时对照的是 **原稿 vs 当前最新一轮**。
...
2. 通过该区块 **「LLM 校对 JSON」**，或右键菜单 / 命令面板选择 `proofread file`。第 1 轮写出 `文档.proofread.1.json`（旧版无序号的 `文档.proofread.json` 仍视为第 1 轮）。
3. 显示当前配置请你确认（含轮次、输出路径、target 来源）。配置说明见上文。
4. 可中途取消。未完成条目在结果里为 `null`；按钮会变成 **「LLM 校对 JSON - 续跑」**，只处理未完成项。切分 JSON 与结果条数不一致时会提示备份或手动处理。
5. 第 1 轮全部完成后，「LLM 校对 JSON」禁用。要用上一轮校对后的全文再打一遍，点 **「重叠校对 JSON」**（命令 `overlay proofread file`），写出 `文档.proofread.2.json`、`.3.json` …；`reference` / `context` 仍取自原始 `文档.json`。未完成时按钮为 **「重叠校对 JSON - 续跑」**。
6. 重叠校对 = 用上一轮全文结果再校一轮，不是滑窗切分。要从原稿重跑第 1 轮：删除校对结果文件后再用「LLM 校对 JSON」。同一轮不能有多份结果（例如无序号的 `.proofread.json` 与 `.proofread.1.json` 并存）。已有第 2 轮及以后时，「LLM 校对 JSON」拒绝，以免再写一份第 1 轮与更高轮并排。重叠校对升号只看最高轮是否已完成；续跑未完成的最高轮时需要紧邻上一轮存在且完整，否则请自行补上或删除该轮结果。
7. 校对结果区标明当前轮次；比较前后差异为原稿 vs 该轮拼合稿。
...
5. 测试.proofread.1.json（以及 .2.json、.3.json …），各轮校对结果；`null` 表示未完成，续跑只处理这些条目。旧版无序号的 `测试.proofread.json` 视为第 1 轮（仅当不存在 `.1.json`）。同一轮不能有多份；缺中间轮不阻止从最高已完成轮继续升号。
6. 测试.proofread.1.json.md，该轮结果的拼合稿，与最初的 markdown 比较即可看出改动（原稿 vs 第 N 轮）；若已存在则自动备份并加时间戳
7. 测试.diff.html：通过jsdiff库比较校对前后markdown文件所得的结果，与Word近似的行内标记，可通过浏览器打印成PDF。需要联网调用jsdiff库，并等待运算完成
8. 测试.proofread.log，校对日志（各轮追加，记录 Round 与 target 来源），**校对文本选段的结果也会存在这里**
...
11. 条目模式另有 `测试.proofread.1-item.json`（第 N 轮为 `测试.proofread.N-item.json`）

**请特别注意：除自动累加的日志文件和提示备份的校对结果 JSON / 自动备份的拼合 `.json.md`，其余中间文件，每次操作都将重新生成！如有需要，请自行备份。**
...
精排与预筛可在配置中选择跟随「校对」或「参考资料规划」。点击树中某一项即可选择平台、填写模型、切换跟随，或**单独开关思考模式**（可在跟随平台/模型的同时覆盖思考）。配置思考时侧栏与菜单会提示：当前生效、产品默认、开启的收益与负担。填写模型时会提示可用更低价档位。
...
- **校对**：`qwen3.7-max`（文本质量优先）；日常建议关闭思考
- **参考资料规划 / 预deepseek-flash选用价格较低的模型以控制成本；复杂书稿可对规划单独开思考
- **预筛 / 精排**：默认跟随参考资料规划，建议保持关思考
- **编辑记忆合并**：默认跟随校对；需要时再开思考

**较低价模型示例**（大致由低到高；各平台实际价目以厂商为准）：`qwen3.7-flash`、`deepseek-flash`、`qwen3.8-flash`、`qwen3.7-plus`、`deepseek-v4-pro`。规划、预筛、精排等多轮调用尤其适合改用这些档位。
...
1. 百炼接入域名迁到业务空间专属域名（2026-09-30 起共享域名进入维护，现有调用仍可用）。见 [docs/bailian-workspace-domain-migration.md](docs/bailian-workspace-domain-migration.md)
2. 翻译流程
    1. 结构树式切分
    2. 抽样调查
    3. 流式处理与翻译memo
    4. 前后段落（及其译文）作为语境（可选用锚点标签代替target，三个标签的语义要明确）
3. 缓存命中优化（加段落标记/空行？加标签？）
4. 优化资料查询：接入 web 搜索服务
5. 分词连写检查提示词
6. 优化memo管理智能体
    1. 三段式：原文；改后；说明
    2. 自动分类，加类别说明
    ...
    3. 存档、压缩工作流……
7. 本地视觉模型图像校对工作流（专注于图像审查、版式和空间关系）
8.  尝试用思考模型校对
9.  预置更多提示词
...
11. 读秀在线引文核对
12. 人看的相似度使用编辑距离：fastest-levenshtein，在对齐完成后，快速计算两个相似句子的“修改程度”百分比。它是 JS 环境下编辑距离运算的最快实现
13. 勘误表改为JSON加web viewer
14. 内部git版本管理
