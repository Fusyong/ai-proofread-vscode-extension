...
3. **标题层级与连续性检查**：命令`check numbering hierarchy`。检查标题序号和文中序号的层级与连续性；在侧栏「标题层级树」中可定位到文档、对标题序号执行同级别批量操作：标记为 Markdown 标题、升级、降级。
...
- 特性：标题层级与连续性检查（解析、检查、TreeView 展示；标题序号支持标记为 Markdown 标题、升级、降级）
