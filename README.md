...
1. 本扩展命令`mark titles from table of contents`，可以根据一个目录表文件（Markdown分级列表的形式）在当前文档中把标题标记出来，会自动忽略数字（如页码）、页面前导符号（英文句点或省略号）、空格、圈码、Markdown形式的注码。
...
7. [ ] 预置更多提示词，包括常用的专项校对
    ...
    5. [ ] 翻译
    6. [ ] 按小学语文教材标准加拼音
8. [ ] 数字连续性检查（以Python库相应模块为基础）
9.  [ ] 引文核对（以Python库相应模块为基础）
10. [ ] 自主发现、提出、校对知识性问题
    1. [ ] 检索、核对互联网资料
