...
export function parseReferencePrepPlan(raw: string, allowedIntents: ReferencePrepIntent[]): ReferencePrepPlan {
    ...
    for (let i = 0; i < queriesRaw.length; i++) {
        ...
        if (x?.web && typeof x.web === 'object') {
            const searchTerms = Array.isArray(x.web.searchTerms)
                ? x.web.searchTerms
                      .map((s: unknown) => (typeof s === 'string' ? s.trim() : ''))
                      .filter(Boolean)
                : [];
            if (searchTerms.length > 0) {
                q.web = {
                    searchTerms: searchTerms.slice(0, 3),
                    why: typeof x.web.why === 'string' ? x.web.why : undefined,
                };
            }
        }
        if (q.dict || q.grep || q.wikipedia || q.web) {
            queries.push(q);
        }
    }
