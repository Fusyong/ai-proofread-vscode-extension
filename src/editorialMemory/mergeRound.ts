...
2. JSON 键：global_md（字符串，对应 ## 全局 下的 bullet 正文，**不要**写 ## 全局 标题行）、sections（数组，每项 { path, body_md }；path 必须与下面给出的允许 path 之一**完全一致**，或是当前 heading_path）、classification_notes（可选）、recent_append（可选单行，写入 ## 近期记忆 首条；与程序摘要去重，可省略）。
...
6. 禁止大段复述书稿原文；条目优先「忌…→宜…」短句（语义与知识为主，少纯标点版式）。
