...
export const SLOT_TABLE: SlotDef[] = [
    ...
    { slotId: 4, baseLevel: 3, sequenceType: 'chinese-lower', marker: '顿', pattern: /^\s*(#{1,6}\s+)?(?<![\u4e00-\u9fa5])([一二三四五六七八九十百千]+)、\s*(.*)$/, multiLevel: false },
    { slotId: 15, baseLevel: 3, sequenceType: 'chinese-lower', marker: '第逗', pattern: /^\s*(#{1,6}\s+)?(?<![\u4e00-\u9fa5])第([一二三四五六七八九十百千]+)([，,])\s*(.*)$/, multiLevel: false },
    { slotId: 16, baseLevel: 3, sequenceType: 'chinese-lower', marker: '其逗', pattern: /^\s*(#{1,6}\s+)?(?<![\u4e00-\u9fa5])其([一二三四五六七八九十百千]+)([，,])\s*(.*)$/, multiLevel: false },
    ...
    { slotId: 13, baseLevel: 5, sequenceType: 'arabic', marker: '点单', pattern: /^\s*(#{1,6}\s+)?(?<![0-9])(\d+)\.(?![0-9])\s*(.*)$/, multiLevel: false },
    { slotId: 14, baseLevel: 5, sequenceType: 'latin-upper', marker: '点单', pattern: /^\s*(#{1,6}\s+)?(?<![A-Z])([A-Z])\.(?![A-Z])\s*(.*)$/, multiLevel: false },
