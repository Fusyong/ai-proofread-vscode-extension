    ...
    public async handleWebviewMessage(message: any, panel: vscode.WebviewPanel, context: vscode.ExtensionContext): Promise<void> {
        ...
        try {
            switch (command) {
                ...
                case 'replaceFormFeeds': {
                    await runWithWorkingEditor('ai-proofread.replaceFormFeeds');
                    break;
                }
                case 'opencc': {
        ...
        return `
            <div class="process-section">
                ...
                <button class="action-button" onclick="handleAction('convertPdfToMarkdown')" title="${commandHoverTitle('将 PDF 转为 Markdown', 'ai-proofread.convertPdfToMarkdown')}">PDF → Markdown</button>
                    ...
                    <button class="action-button" onclick="handleAction('replaceFormFeeds')" title="${commandHoverTitle('将 pdftotext 分页符替换为页码注释/标题，并可清除页脚页码行', 'ai-proofread.replaceFormFeeds')}">替换分页符</button>
                    ${sep}
